...
class OrderController extends Controller
{
    ...
    public function uploadSlip(Request $request, $id)
    {
        $request->validate([
            ...
            'slip.required' => 'กรุณาแนบรูปภาพสลิป',
            'slip.image' => 'ไฟล์ที่แนบต้องเป็นรูปภาพเท่านั้น',
            'slip.mimes' => 'รองรับเฉพาะไฟล์รูปภาพนามสกุล .jpeg, .png, .jpg เท่านั้น',
            'slip.max' => 'ขนาดไฟล์รูปภาพต้องไม่เกิน 5MB',
        ]);
        ...
        if ($request->hasFile('slip')) {
            // ถ้ามีรูปเก่าอยู่แล้ว ให้ลบทิ้งก่อน
            if ($order->payment_slip && Storage::disk('public')->exists($order->payment_slip)) {
                Storage::disk('public')->delete($order->payment_slip);
            }

            // ตั้งชื่อไฟล์และบันทึก
            $fileName = 'slip_' . $order->order_number . '_' . time() . '.' . $request->file('slip')->extension();
            $path = $request->file('slip')->storeAs('slips', $fileName, 'public');
            
            $order->payment_slip = $path;
            $order->payment_status = 'reviewing'; // 🌟 เปลี่ยนสถานะการจ่ายเงินเป็น "รอตรวจสอบ"
            $order->save();
    ...
    public function switchToCash($id)
    {
        ...
        OrderLog::create([
            ...
            'old_status' => $order->status,
            'new_status' => $order->status,
            'note' => 'ลูกค้าเปลี่ยนวิธีชำระเงินเป็นเงินสดปลายทาง'
        ]);

            ...
            <form action="{{ route('customer.book.store') }}" method="POST" class="lg:grid lg:grid-cols-12 lg:gap-8 pb-24 lg:pb-0" id="booking-form">
                ...
                <div class="lg:col-span-8 space-y-8">
                            ...
                            @foreach ($packages as $package)
                                <label class="cursor-pointer relative group block h-full">
                                    <input
                                        ...
                                        class="peer sr-only package-option"
                                        required
                                    ...
                                    <div class="h-full p-6 rounded-2xl border-2 border-gray-100 dark:border-slate-600 bg-gray-50/50 dark:bg-slate-700/30 hover:border-pink-300 dark:hover:border-pink-500 peer-checked:border-pink-500 peer-checked:bg-pink-50/50 dark:peer-checked:bg-slate-700/80 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-md text-center flex flex-col justify-between">
                                        <div>
                                            <div class="w-14 h-14 mx-auto bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-sm mb-4 transition-colors peer-checked:shadow-pink-200">
                                                <i class="fa-solid fa-shirt text-2xl text-gray-400 dark:text-gray-500 peer-checked:text-pink-500 transition-colors"></i>
                                            </div>
                                            ...
                                            <div class="addon-card relative p-4 rounded-2xl border-2 border-gray-100 dark:border-slate-600 bg-white dark:bg-slate-800 hover:border-pink-300 dark:hover:border-pink-500 transition-all duration-300 cursor-pointer shadow-sm hover:shadow-md flex flex-col h-full justify-between gap-4"
                                                ...
                                                <div class="flex items-start gap-3 w-full">
                                                    ...
                                                    <div class="flex-1 min-w-0 break-words">
                                                        <p class="font-bold text-gray-800 dark:text-gray-100 text-sm md:text-base leading-snug mb-1">{{ $addon['name'] }}</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pl-0 md:pl-14">
                            @foreach ($timeSlots as $slot)
                                <label class="cursor-pointer relative group">
                                    <input type="radio" name="time_slot_id" value="{{ $slot->id }}" class="peer sr-only" required {{ old('time_slot_id') == $slot->id ? 'checked' : '' }}>
                                    <div class="px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-slate-600 bg-gray-50/50 dark:bg-slate-700/30 hover:border-pink-300 peer-checked:border-pink-500 peer-checked:bg-pink-50 dark:peer-checked:bg-slate-700 transition-all duration-200 flex items-center justify-between group-hover:shadow-sm">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-bold text-gray-700 dark:text-gray-200">{{ $slot->round_name }}</span>
                                        </div>
                    ...
                    <div class="rounded-3xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 md:p-8 shadow-sm">
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 tracking-tight">ตั้งค่าการซักและอบ</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-0 md:pl-14">
                            <div class="space-y-2">
                                ...
                                <select name="wash_temp" class="w-full px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-slate-600 bg-gray-50 dark:bg-slate-700/50 text-gray-800 dark:text-gray-100 focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 outline-none transition-all cursor-pointer font-medium appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23cbd5e1%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_auto] bg-no-repeat bg-[position:right_1.25rem_center]">
                                    <option value="เย็น" {{ old('wash_temp') === 'เย็น' ? 'selected' : '' }}>❄️ ซักน้ำเย็น (ถนอมผ้า)</option>
                                    <option value="อุ่น" {{ old('wash_temp', 'อุ่น') === 'อุ่น' ? 'selected' : '' }}>🌡️ ซักน้ำอุ่น (คราบทั่วไป)</option>
                                    <option value="ร้อน" {{ old('wash_temp') === 'ร้อน' ? 'selected' : '' }}>🔥 ซักน้ำร้อน (ฆ่าเชื้อโรค)</option>
                                </select>
                            </div>
                            <div class="space-y-2">
                                <label class="block font-bold text-gray-700 dark:text-gray-300">
                                ...
                                <select name="dry_temp" class="w-full px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-slate-600 bg-gray-50 dark:bg-slate-700/50 text-gray-800 dark:text-gray-100 focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 outline-none transition-all cursor-pointer font-medium appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23cbd5e1%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_auto] bg-no-repeat bg-[position:right_1.25rem_center]">
                                    <option value="อุ่น" {{ old('dry_temp', 'อุ่น') === 'อุ่น' ? 'selected' : '' }}>☁️ อบลมอุ่น (ถนอมผ้า)</option>
                                    <option value="ปานกลาง" {{ old('dry_temp') === 'ปานกลาง' ? 'selected' : '' }}>☀️ อบความร้อนปานกลาง</option>
                                    <option value="ร้อน" {{ old('dry_temp') === 'ร้อน' ? 'selected' : '' }}>🔥 อบความร้อนสูง (ผ้าหนา)</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-3xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 md:p-8 shadow-sm">
                        <div class="flex items-center gap-4 mb-6">
                            <div class="shrink-0 bg-gradient-to-br from-pink-400 to-pink-600 text-white w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-md text-lg">6</div>
                            <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 tracking-tight">สถานที่รับ-ส่งผ้า</h3>
                        </div>

                        <div class="pl-0 md:pl-14">
                            <div class="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-4 mb-5 rounded-r-xl shadow-sm">
                                <div class="flex items-start gap-3">
                                    <i class="fa-solid fa-location-dot text-blue-500 mt-1 text-lg"></i>
                                    <p class="text-sm text-blue-800 dark:text-blue-200 leading-relaxed">
                                        ระบบจะอ้างอิงพิกัด GPS จากที่อยู่หลักของคุณ หากต้องการเปลี่ยนสถานที่รับ-ส่งผ้าแบบถาวร 
                                        <a href="{{ route('customer.profile') }}" class="font-bold underline decoration-blue-400 underline-offset-2 hover:text-blue-600 transition-colors">ไปที่หน้าโปรไฟล์</a>
                                    </p>
                                </div>
                            </div>

                            <label class="block font-bold text-gray-700 dark:text-gray-300 mb-2">ที่อยู่ปัจจุบัน / จุดสังเกตเพิ่มเติมให้ไรเดอร์</label>
                ...
                <aside class="lg:col-span-4 mt-8 lg:mt-0 relative">
                    <div class="lg:sticky lg:top-24 space-y-5">
                        <div class="rounded-3xl bg-white dark:bg-slate-800 border-2 border-pink-100 dark:border-slate-700 shadow-xl shadow-pink-100/50 dark:shadow-none overflow-hidden">
                            <div class="bg-pink-50 dark:bg-slate-700/50 px-6 py-4 border-b border-pink-100 dark:border-slate-600">
                            ...
                            <div class="p-6 space-y-4 text-base">
                                <div class="flex justify-between items-center text-gray-600 dark:text-gray-300">
                                    ...
                                    <span id="summary-subtotal" class="font-medium text-gray-800 dark:text-gray-100">฿0</span>
                                </div>
                                <div class="flex justify-between items-center text-gray-600 dark:text-gray-300">
                                    <span>เมนูเสริม</span>
                                    <span id="summary-addon" class="font-medium text-gray-800 dark:text-gray-100">฿0</span>
                                </div>
                                <div class="pt-4 mt-2 border-t border-dashed border-gray-200 dark:border-slate-600 flex justify-between items-end">

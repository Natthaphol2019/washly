...
<body class="bg-slate-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200 font-sans antialiased pb-20 md:pb-0 transition-colors duration-300">
            ...
            <div class="flex items-center gap-2 md:gap-3">
                ...
                <div class="relative">
                    <button type="button" id="notif-toggle"
                        class="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-pink-100 hover:text-pink-600 dark:hover:bg-slate-600 transition-colors shadow-sm relative">
                        <i class="fa-solid fa-bell text-sm sm:text-base"></i>
                        <span id="notif-badge"
                            class="hidden absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[9px] sm:text-[10px] font-bold px-1 sm:px-1.5 py-0.5 rounded-full border-2 border-white dark:border-slate-800">0</span>
                    </button>

                    <div id="notif-panel"
                        class="hidden absolute right-0 mt-2 w-72 sm:w-80 max-w-[85vw] sm:max-w-[90vw] bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl sm:rounded-2xl shadow-xl z-50 overflow-hidden">
                        <div class="px-3 sm:px-4 py-3 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
                            <p class="font-semibold text-xs sm:text-sm">การแจ้งเตือน</p>
                            <button type="button" id="notif-read-all" class="text-xs text-pink-500 hover:text-pink-600">อ่านแล้วทั้งหมด</button>
                        </div>
                        <div id="notif-list" class="max-h-72 sm:max-h-80 overflow-y-auto divide-y divide-gray-100 dark:divide-slate-700"></div>
                ...
                <a href="{{ route('customer.profile') }}" class="hidden md:flex items-center gap-2 pl-1 pr-2.5 sm:pr-3 py-1 rounded-full bg-pink-50 dark:bg-slate-800 border border-pink-100 dark:border-slate-700 hover:shadow-sm hover:border-pink-300 dark:hover:border-slate-500 transition-all group">
                    <div class="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gradient-to-tr from-pink-400 to-rose-300 flex items-center justify-center text-white font-bold text-xs shadow-sm group-hover:scale-110 transition-transform">
                        {{ mb_substr(Auth::user()->fullname ?? 'C', 0, 1) }}
                    </div>
                    <span class="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200 group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">
                        {{ explode(' ', Auth::user()->fullname)[0] ?? 'ลูกค้า' }}
                    </span>
                </a>

                <button onclick="toggleTheme()" id="theme-toggle-btn" class="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-pink-100 hover:text-pink-600 dark:hover:bg-slate-600 transition-colors shadow-sm">
                    <i class="fa-solid fa-moon text-sm sm:text-base"></i>
                </button>

                <form action="{{ route('logout') }}" method="POST" class="m-0">
                    @csrf
                    <button type="button" onclick="confirmLogout(this.closest('form'))" class="hidden md:flex items-center gap-2 text-red-500 dark:text-red-400 hover:text-white bg-red-50 dark:bg-red-500/10 hover:bg-red-500 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all shadow-sm">
                        <i class="fa-solid fa-right-from-bracket"></i>
                        <span>ออก</span>
                    </button>
                    <button type="button" onclick="confirmLogout(this.closest('form'))" class="md:hidden text-red-500 dark:text-red-400 w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors shadow-sm">
    ...
    <main class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 min-h-screen">
        @yield('content')
    </main>
